...
fun GameBoard<Int?>.addNewValue(initializer: Game2048Initializer<Int>) {
    /* remember: initializer returns Pair(randomCell, randomValue) */
...
fun GameBoard<Int?>.moveValuesInRowOrColumn(rowOrColumn: List<Cell>): Boolean {
    val list: List<Int> = rowOrColumn.map { this[it] }.moveAndMergeEqual { it * 2 }
...
fun GameBoard<Int?>.moveValues(direction: Direction): Boolean {
    val range = 1..width
    val dir = if (direction in listOf(Direction.UP, Direction.LEFT)) range else range.reversed()
    var didWeMove = false
