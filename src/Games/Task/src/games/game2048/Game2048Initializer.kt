...
object RandomGame2048Initializer: Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        /*
         * - given a board of width w, it starts out all cells(i, j) holding 'null'
         * - get a random cell on the board that is null, assign its value to generateRandomStartValue()
         * */
        val emptyCells = board.getAllCells().filter { board[it] == null }
        if (emptyCells.isEmpty()) return null
        val rand = generateRandomStartValue()
        val randomCell = emptyCells.shuffled().first()
        return Pair(randomCell, rand)
    }
}